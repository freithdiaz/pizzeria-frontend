...
async function cargarGruposBebidas(productoId) {
    try {
        const response = await fetch(`/api/productos/${productoId}/grupos-bebidas`);
        const data = await response.json();
        return data.grupos || [];
    } catch (error) {
...
async function cargarGruposAdiciones(productoId) {
    try {
        ...
        const response = await fetch(`/api/productos/${productoId}/grupos-adiciones`);
        let gruposNormales = [];
        ...
        const vinculacionesResponse = await fetch(`/api/productos/${productoId}/vinculaciones`);
...
async function buscarClientePorTelefonoInline() {
    ...
    try {
        ...
        const response = await fetch(`/api/delivery/usuario/por-telefono?telefono=${encodeURIComponent(telefono)}`);
        const data = await response.json();
...
async function buscarDireccionAutocompletado(query) {
    ...
    sugerenciasDireccionTimeout = setTimeout(async () => {
        try {
            const response = await fetch(`/api/delivery/suggestions?q=${encodeURIComponent(query)}`);
            const data = await response.json();
...
async function calcularPrecioDomicilio(municipio, barrio) {
    try {
        ...
        const response = await fetch(`/api/delivery/calcular-precio?${params}`);
...
async function cargarProductosVinculados(productoId) {
    try {
        const response = await fetch(`/api/productos/${productoId}/productos-vinculados`);
